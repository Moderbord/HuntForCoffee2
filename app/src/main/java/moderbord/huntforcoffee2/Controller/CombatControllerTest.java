...
public class CombatControllerTest extends EventController {
    ...
    private Entity first = new EntityBuilder().seteName("Philip").seteGender(C.GENDER_MALE).seteMaxHealth(55).seteHealth(55).seteMana(60).seteMaxMana(60).seteAgility(25).seteQuickness(36).seteAlly(true).createEntity();
    private Entity second = new EntityBuilder().seteName("Lögdal").seteGender(C.GENDER_MALE).seteMaxHealth(70).seteHealth(50).seteMana(4000).seteMaxMana(40).seteAgility(70).seteQuickness(45).seteAlly(true).createEntity();
    ...
    private Entity fourth = new EntityBuilder().seteName("Hanna").seteGender(C.GENDER_FEMALE).seteMaxHealth(50).seteHealth(50).seteMana(120).seteMaxMana(120).seteAgility(23).seteQuickness(41).seteAlly(true).createEntity();
    ...
    public void prepareCombat() {
        ...
        Weapon rClaw = new WeaponBuilder().setWepType(C.WEAPON_TYPE_CLAWS).setGearSlot(C.GEAR_SLOT_MAIN_WEP).setName("left claw").setMinDmg(7).setMaxDmg(15).createWeapon();
        Weapon lClaw = new WeaponBuilder().setWepType(C.WEAPON_TYPE_CLAWS).setGearSlot(C.GEAR_SLOT_OFF_WEP).setName("right claw").setMinDmg(7).setMaxDmg(15).createWeapon();
    ...
    public void targetSelection() {
        ...
        boolean casterIsAlly = caster.isAlly();
        for (Entity e : entityList) {
    ...
    private void performEnemyNormalAttack(){ // TODO rework system to first check for threat, low health, high health, spellcaster , etc..
        ...
        if (targetList.isEmpty()){
            ui.button8.performClick(); // Wait action
        } else {
            int x = Utils.rand.nextInt(targetList.size());
            target = targetList.get(x);
            loadedAction = NormalAttack.getInstance();
            targetList.clear();
            result();
        }
    ...
    private boolean possibleTarget(Entity e){
        CombatStats cs = e.getCombatStats();
        boolean casterIsAllied = caster.isAlly();
        boolean possibleTarget = !cs.isDefeated() && cs.isActive() && cs.isTargetable() && !e.isAlly() == casterIsAllied;
